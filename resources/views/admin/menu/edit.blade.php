                      ...
                      <div class="col-lg-12 col-xl-12">
                        <div class="tab-content" id="pills-tabContent">
                          <div class="tab-pane fade show active" id="pills-input" role="tabpanel" aria-labelledby="pills-input-tab">
                            <form method="POST" action="{{ route('menu.update', $data->id) }}"  class="theme-form">
                              @csrf
                              ...
                              <div class="mb-3 draggable">
                                ...
                                <select class="form-control form-control-primary btn-square" value="{{ $data->slug }}" name="slug">
                                  @foreach ($pages as $u)
                                     <option value="{{ $u->slug }}" @if($data->slug === $u->slug) selected @endif >{{ $u->slug }}</option>
                                  @endforeach
                               </select>
                              </div>
                              <div class="mb-3 draggable">
                                <label for="input-text-1">PARENT</label>
                                <select class="form-control form-control-primary btn-square" value="{{ $data->parent }}" name="parent">
                                  @foreach ($menus as $u)
                                     <option value="{{ $u->name }}" @if($data->parent === $u->name) selected @endif>{{ $u->name }}</option>
                                  @endforeach
                               </select>
                              </div>
